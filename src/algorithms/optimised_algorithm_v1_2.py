...
def get_best_portfolio(data_csv):
    ...
    for i in range(1, len(actions_data_sorted_under_max_purchase_cost)):
        if (get_portfolio_cost(best_portfolio) + actions_data_sorted_under_max_purchase_cost[i][
            1] > MAXIMUM_PURCHASE_COST) and (
                get_portfolio_cost(best_portfolio[:-1]) + actions_data_sorted_under_max_purchase_cost[i][
                1] <= MAXIMUM_PURCHASE_COST):
            if best_portfolio[-1][1] * best_portfolio[-1][2] < actions_data_sorted_under_max_purchase_cost[i][1] * \
                    actions_data_sorted_under_max_purchase_cost[i][2]:
                best_portfolio.pop()
